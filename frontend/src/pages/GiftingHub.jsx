...
const CATEGORIES = ['All', 'Magazines', 'Bouquets', 'Hampers', 'Keychains', 'Trousseau Packing', 'Crochet'];
...
const CAT_META = {
  'Magazines':          { emoji: '📖', desc: 'Custom memory books with your photos & words' },
...
export default function GiftingHub() {
  ...
  // Limit display list down to exactly 4 items total for the homepage layout
    ...
    <div style={{ background: '#FFF8FA', minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ textAlign: 'center', padding: '0 20px', position: 'relative', zIndex: 1, maxWidth: '700px', width: '100%' }}>
          ...
          {/* Quick categories navigation links directly to catalog endpoint */}
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {CATEGORIES.slice(1).map(c => (
              <button key={c} onClick={() => { window.location.href = `/products?category=${encodeURIComponent(c)}`; }} style={{
      ...
      {/* Latest Collection limited internally to 4 items */}
      <LatestCollection limit={4} />
        ...
        <section style={{ padding: 'clamp(48px,6vw,80px) 0', background: '#FFFFFF' }}>
          <div className="container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(45%, 280px), 1fr))', gap: '24px' }}>
              {featured.slice(0, 4).map(p => <ProductCard key={p._id} product={p} featured />)}
            </div>
          </div>
        </section>
      )}
      ...
      <section id="products-section" style={{ padding: 'clamp(48px,6vw,80px) 0' }}>
        <div className="container">
          ...
          {/* Conditional Filters Block: Automatically hides entirely on mobile devices */}
          {!isMobile && (
          ...
          {loading ? (
            ...
            <>
              {!isMobile && (
                <p style={{ marginBottom: '28px', color: '#8B6070', fontSize: '13px' }}>Showing <strong style={{ color: '#2A0A12' }}>{visibleProducts.length}</strong> of {filtered.length} products</p>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(45%, 280px), 1fr))', gap: '24px' }}>
              ...
              <div style={{ textAlign: 'center', marginTop: '48px' }}>
                <a href="/products" style={{
                  ...
                  background: '#6B1A2A', color: '#fff', borderRadius: '40px', fontSize: '15px', fontWeight: 600,
                ...
                >
                  See all Products <span style={{ fontSize: '18px' }}>&rarr;</span>
                </a>
              </div>
            </>
          )}
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(45%, 180px), 1fr))', gap: '16px' }}>
            {Object.entries(CAT_META).map(([name, meta]) => (
              <button key={name} onClick={() => { window.location.href = `/products?category=${encodeURIComponent(name)}`; }} style={{
      ...
      <section style={{ padding: 'clamp(56px,7vw,90px) 0', background: 'linear-gradient(135deg, #6B1A2A 0%, #C4758A 100%)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          >
            <span style={{ fontSize: '20px' }}><WAIcon /></span> Chat on WhatsApp
          </a>
        </div>
      </section>
      ...
      <div style={{ padding: '20px 22px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <a href={link} target="_blank" rel="noopener noreferrer" style={{
            ...
            <span style={{ fontSize: '15px' }}><WAIcon /></span> WhatsApp
          </a>
        </div>
      </div>
    </div>
